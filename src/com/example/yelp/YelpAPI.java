...
public class YelpAPI {
	...
	public ArrayList<Place> getPlaces(Location pLoc, String searchTerm) {
		...
		Yelp m_Yelp = new Yelp(searchTerm, pLoc.getLatitude(), pLoc.getLongitude());
		m_Yelp.execute();
		...
		try {
			...
			for (int i = 0; i < businesses.length(); ++i) {
				...
				for (int j = 0; j < m_displayAddress.length(); ++j) {
					m_Address.append(m_displayAddress.get(j).toString() + "\n");
				}

				String m_Name = m_business.get("name").toString();
				double m_Rating = Float.parseFloat(m_business.get("rating").toString());
				double m_Distance = Double.parseDouble(m_business.get("distance").toString()) / 1609.34;
				Place newPlace = new Place(m_Name, m_Rating, m_Address.toString(), m_Distance);
				Log.d("DEBUG", m_Name);
